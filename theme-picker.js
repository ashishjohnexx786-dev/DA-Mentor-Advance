document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(modal&&!gallery){
    const themeLabel=settings?.closest('label');if(themeLabel)themeLabel.style.display='none';
    gallery=document.createElement('div');gallery.id='advanceSettingsThemes';gallery.className='proSettingsThemes';
    gallery.innerHTML=`<span class="muted tiny">Appearance</span><div class="proSettingsThemeGrid">${Object.keys(names).map(k=>`<button type="button" class="proSettingsThemeCard" data-advance-settings-theme="${k}"><i style="background:${swatches[k]}"></i><span><b>${names[k]}</b><small>${desc[k]}</small></span></button>`).join('')}</div>`;
    const saveRow=document.getElementById('saveSettingsBtn')?.parentElement;modal.insertBefore(gallery,saveRow||null);
  }
  ...
  function sync(){const t=selected();document.body.dataset.theme=t;if(settings)settings.value=t;document.querySelectorAll('[data-advance-settings-theme]').forEach(b=>b.classList.toggle('active',b.dataset.advanceSettingsTheme===t))}
